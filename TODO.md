...
- **Recommended action:** When `readiness.canGenerate` is true and no package exists, show an explicit Generate action; call `requestPackageGeneration` with a stable idempotency key; model generating, failure/retry, and success states without losing the last successful content. Extend the local fixture so an eligible applicant-completed case returns a deterministic generated package instead of 501.
- **Tests:** Add a package/API test proving an eligible non-pre-generated case can be generated idempotently, plus an XCUITest journey that clears the final blocker, triggers generation, and reaches the verification/output screen. Assert that a generation failure remains retryable and never masquerades as “review required.”
- **Dependencies:** none. Production generation remains broader roadmap work, but the local Alpha 0.2 flow can and should be internally complete and honest.
- **Resolution:** Ready cases now expose a localized Generate action with a stable retry idempotency key and explicit progress/error states. The stub fails closed when review data is absent, creates and persists a deterministic package once all required values are human-confirmed, and returns that package on replay. Generated demo artifacts are visibly marked **NOT FOR FILING**; production-backed official form generation remains roadmap work.
- **Verification:** Package coverage proves eligible generation, idempotent replay, case-state transition, and relaunch persistence. `testFullyReviewedCaseGeneratesPackage` confirms the complete applicant journey through the generated output screen and is part of the critical PR UI set.

### T-59 · Files and Print actions share only the package manifest
- **Priority:** High · **Category:** Bug / Export · **Status:** Complete (2026-08-11), pending CI
- **Description:** `PackageView.swift:122-130` routes both `.files` and `.print` to `makeExportManifest`; `:201-235` creates only `LaPluma-Package-Manifest.txt`. Neither path exports the `PDFOutput` artifacts or calls `api.export` for its channel. “Save to Files” saves an inventory, and “Print” prints that inventory, contradicting both the labels and `docs/08-ux-design.md:596-597`. (CODE_REVIEW H-12)
- **Recommended action:** Extend the package/client contract with retrievable artifact bytes or protected local file URLs. Export the whole package through the Files document flow and send the PDF documents through AirPrint, preserving the existing scratch-file protection and cleanup guarantees. Until artifacts are available, hide or clearly disable these actions rather than presenting the manifest as a successful package export.
- **Tests:** Add contract tests for artifact identity/integrity and UI coverage that verifies Files receives PDFs (or a documented package archive) and Print receives printable PDF data. Keep secure-link coverage separate because that channel already has a distinct delivery contract.
- **Dependencies:** T-58 for the applicant-completed generation journey; production-backed artifacts remain part of `MOBILE_NEXT_TASKS.md` § Document services, but no user decision or credential is required to make the local fixture behavior truthful.
- **Resolution:** The export contract now distinguishes byte-bearing package artifacts from secure delivery links. The local client creates one valid, multi-page PDF artifact per package, caches it so Files and Print receive identical bytes, and reports its MIME type, page count, and SHA-256 digest. The app verifies all four properties before presenting a protected copy to the Files document picker or the PDF bytes to AirPrint; secure delivery remains a separate link flow. Export filenames are also constrained to a single path component before scratch-file creation.
- **Verification:** Contract tests validate the PDF signature, digest, page count, cross-channel identity, and secure-link result. The export XCUITest exercises the Files picker, AirPrint sheet, and secure delivery independently.
