...
| Automated tests | Package and UI suites verified | 74 package tests; five critical PR UI journeys only for UI-relevant changes; all serial journeys on weekday/manual regression; static policy checks; marketing-fixture privacy gates; unsigned Release archive validation; failure-only `.xcresult` retention | Add device-farm and physical-device coverage; monitor Apple iOS 26.5 simulator warnings in `IOS_CI_WARNINGS.md` |
