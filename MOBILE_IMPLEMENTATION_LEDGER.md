...
| `APERTURE_ENVIRONMENT` | Selects which deployment the app talks to | No | `local`, `development`, `staging`, `production` | Mobile | `Info.plist` via `project.pbxproj`; `ApertureEnvironment.swift` | Local development and every deployed build | Confirmed | Default: `development` (GCP `lapluma-dev-gcp`) |
...
| `APERTURE_API_BASE_URL` | Base URL the generated client issues requests against | No | `https://<gateway-id>-<hash>.<region>.gateway.dev` | Platform/API | `Info.plist` via `project.pbxproj`; `ApertureEnvironment.swift` | Dev and production API calls | Confirmed | Live dev API Gateway: `https://lp-gateway-dev-2pou78uy.uc.gateway.dev` |
| `APERTURE_WORKFLOW_API_BASE_URL` | Base URL for Workflow and Workforce endpoints | No | `https://<gateway-id>-<hash>.<region>.gateway.dev` | Platform/API | `Info.plist` via `project.pbxproj`; `ApertureEnvironment.swift` | Dev and production Workflow API calls | Confirmed | Live dev API Gateway: `https://lp-gateway-dev-2pou78uy.uc.gateway.dev` |
| `APERTURE_WEBAUTHN_RP_ID` | Relying-party domain that scopes every passkey | No | Registrable auth domain, never the API URL | Identity/Security | `ApertureApp.entitlements`; `AppleAppSiteAssociation.swift` | Passkey registration and sign-in | Confirmed | `lp-gateway-dev-2pou78uy.uc.gateway.dev` (dev) / `lapluma.ai` (prod) |
| `APERTURE_ASSOCIATED_DOMAIN` | Entitlement binding the app to the passkey domain | No | `webcredentials:<rp-id>` | Identity/Mobile | `ApertureApp.entitlements` | Passkey registration and sign-in | Confirmed | `webcredentials:lp-gateway-dev-2pou78uy.uc.gateway.dev?mode=developer`, `webcredentials:lapluma.ai` |
| `APERTURE_APP_ATTEST_ENVIRONMENT` | Selects Apple's attestation environment for client integrity | No | `development` or `production` | Security/Mobile | `ApertureApp.entitlements` | Production client attestation | Confirmed | `development` |
